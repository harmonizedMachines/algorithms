inputs = """shzsFcPssFhjFssBzdpRcNHNZrpdJdJVJZ
...
RdPMPsmWHmjfMffPcCWrptcprpFTFrFp""".split('\n')

ans = 0
for rucksack in inputs:
    common = set(rucksack[:len(rucksack) // 2]).intersection(set(rucksack[len(rucksack) // 2:]))
    ans += sum(ord(c) - ord('A') + 27 if c.isupper() else ord(c) - ord('a') + 1 for c in common)

print(ans)

ans = 0
for i in range(0, len(inputs), 3):
    common = set(inputs[i]).intersection(set(inputs[i + 1])).intersection(set(inputs[i + 2]))
    ans += sum(ord(c) - ord('A') + 27 if c.isupper() else ord(c) - ord('a') + 1 for c in common)
