inputs = """[[[[],[7,10,6,5],[],[8]],0,1,[[8,10]],4],[[[4,0,3,2,0]],7],[[],3,[[0,2],8,5],[],[[2,10,4,6]]],[3],[5,[1,[8,0],10,1,[4,3,9,9]],1]]
...
def compare(item1, item2):
    ...
    if isinstance(item1, list) and isinstance(item2, list):
        ...
        if not item2:
            return -1
        tmp = compare(item1[0], item2[0]) 
        if tmp == 0:
            return compare(item1[1:], item2[1:])
        return tmp
    if item1 < item2:
        return 1
    elif item1 == item2:
...
for i in range(len(pairs)):
    pair = [json.loads(f) for f in pairs[i].split("\n")]
    if compare(pair[0], pair[1]) != -1:
        ans += i + 1
print(ans)

allpackets = inputs.replace("\n\n", '\n').split('\n')
allpackets = [json.loads(f) for f in allpackets]
allpackets.append([[2]])
allpackets.append([[6]])
allpackets.sort(key=cmp_to_key(compare), reverse=True)
print((allpackets.index([[2]]) + 1) * (1 + allpackets.index([[6]])))